...
class OrderUpdateUnHappyTestCase(OrderBaseTestCase):
    def test_patch_order_invalid_status(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION='Token ' + self.business_token.key)
        self.url = reverse('order-detail', kwargs={'pk': order.pk})
        self.data = {'status': 'invalid_status'}
        response = self.client.patch(self.url, self.data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class OrderCountHappyTestCase(OrderBaseTestCase):

    def test_get_order_count_success(self):
        Order.objects.create(business_user=self.business_user, status='in_progress', customer_user=self.user,
                                     title='Test Order', revisions=3, delivery_time_in_days=5, price=10, features=['Logo Design'], offer_type='basic')
        self.url = reverse(
...
class CompletedOrderCountHappyTestCase(OrderBaseTestCase):
    ...
    def test_get_order_count_success(self):
        Order.objects.create(business_user=self.business_user, status='completed', customer_user=self.user,
                                     title='Test Order', revisions=3, delivery_time_in_days=5, price=10, features=['Logo Design'], offer_type='basic')
        self.url = reverse(
...
class CompletedOrderCountUnHappyTestCase(OrderBaseTestCase):
    ...
    def test_get_order_count_business_user_not_found(self):
        self.url = reverse('completed-order-count',
                           kwargs={'business_user_id': 9999999})
        self.client.credentials(
            HTTP_AUTHORIZATION='Token ' + self.admin_token.key)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code,
